...
export const ToolCard: React.FC<ToolCardProps> = ({ tool, onClick, language }) => {
  return (
    <div 
    ...
    >
      <div className="relative h-48 overflow-hidden bg-white p-6 flex items-center justify-center border-b border-slate-50 flex-shrink-0">
        <img 
          src={tool.imageUrl} 
          alt={tool.name} 
        ...
        <div className="absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-bold bg-white/90 backdrop-blur-sm shadow-sm flex items-center gap-1 border border-slate-100">
          {tool.category === ToolCategory.FREE ? (
            <><Unlock size={12} className="text-green-600" /> <span className="text-green-700">{language === 'uz' ? 'BEPUL' : language === 'ru' ? 'БЕСПЛАТНО' : 'FREE'}</span></>
          ) : (
            <><Lock size={12} className="text-amber-600" /> <span className="text-amber-700">{language === 'uz' ? 'PULLIK' : language === 'ru' ? 'ПЛАТНО' : 'PAID'}</span></>
          )}
        </div>
      ...
      <div className="p-5 flex-grow flex flex-col">
        ...
        {/* Removed h-20 and line-clamp-4 so text is fully visible */}
        <p className="text-slate-600 text-sm mb-4 leading-relaxed">{tool.description[language]}</p>
